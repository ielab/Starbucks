...
def pickle_load(path, embedding_dim):
    with open(path, 'rb') as f:
        reps, lookup = pickle.load(f)
    return reps[:, :embedding_dim], lookup
...
def main():
    ...
    parser.add_argument('--embedding_dim', type=int, default=-1, help='If set, will only load the first N dimensions of the embeddings, -1 means all dimensions')
    ...
    p_reps_0, p_lookup_0 = pickle_load(index_files[0], args.embedding_dim)
    retriever = FaissFlatSearcher(p_reps_0)

    shards = chain([(p_reps_0, p_lookup_0)],
                   map(pickle_load, index_files[1:], [args.embedding_dim] * len(index_files)))
    ...
    q_reps, q_lookup = pickle_load(args.query_reps, args.embedding_dim)
    q_reps = q_reps
